...
class UserCreateForm(FlaskForm):
    ...
    password1 = PasswordField('비밀번호', validators=[
        DataRequired(), Length(max=20), EqualTo('password2', '비밀번호가 일치하지 않습니다')])
    password2 = PasswordField('비밀번호확인', validators=[DataRequired()])
    birth_date = StringField('생년월일', validators=[DataRequired()])
    phone = StringField('핸드폰번호', validators=[DataRequired('핸드폰 번호는 필수입력 항목입니다.')])
